...
| `dicom2json` | `(inputPath, options?) => Result<{data, warnings, source}>` | DICOM to JSON Model — pure JS, no DCMTK binary, ~75x faster; bounded head-read skips bulk bytes for files > 8 MB (`boundedRead: false` to disable); `dcmtkFallback` option retries via binaries. `dicom2jsonFromBuffer(buffer)` is the sync in-memory variant |
