...
async function echoscu({host, port = 104, options = {}}) {
  let command = [process.env.DCMTK_ECHOSCU, host, port, '--debug']
  ...
  //
  if (options.proposeTransferSyntaxes != null) command.push(`-pts ${options.proposeTransferSyntaxes}`)

  //   -ppc  --propose-pc  [n]umber: integer (1..128)
  //           propose n presentation contexts
  if (options.proposePresentationContexts != null) command.push(`-ppc ${options.proposePresentationContexts}`)
